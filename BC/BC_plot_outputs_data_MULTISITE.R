...
## Create plots
   dev.set()
   pdf(paste('BC_outputs_data_MULTISITE',s,'.pdf',sep=""))
   ...
   for (p in outputsMeasured) {
     datap   <- which( data_name[[s]] == as.character(outputNameList[p]) )
     ...
     points(year_start+(data_time[[s]][datap]-0.5)/366, data_value[[s]][datap], col='blue',lwd=2, cex=2)
     arrows(year_start+(data_time[[s]][datap]-0.5)/366,ucl,
            year_start+(data_time[[s]][datap]-0.5)/366,lcl,
            col='blue',lwd=1,angle=90,code=3,length=0.05)
   }
   
   if(dim(database_mm[[s]])[1]>0) {
     for (p in outputsMeasured_mm) {
       datap   <- which( data_mm_name[[s]] == as.character(outputNameList[p]) )
       ...
       points(q5        [1,],q5        [p,],type='l',col='black',lwd=2, lty=3)
       points(q95       [1,],q95       [p,],type='l',col='black',lwd=2, lty=3)
       points(year_start+(data_mm_time[[s]][datap]-0.5)/366, data_mm_value[[s]][datap], col='blue',lwd=2, cex=2)
       arrows(year_start+(data_mm_time[[s]][datap]-0.5)/366,ucl,
              year_start+(data_mm_time[[s]][datap]-0.5)/366,lcl,
              col='blue',lwd=1,angle=90,code=3,length=0.05)
     }
   }
